...
describe('User Model', () => {
    ...
    describe('Save a new user', () => {
        ...
        it('should not save a new user in the database if the user does not have a password', async () => {
            const { username, email } = testAccount;
        ...
        it('should save a new user in the database if the user has a valid username, email, and password', async () => {
            const { username, email, password } = testAccount;
            
            await (new User({ username, email, password })).save();

            const user = await User.findOne({ username });
            const hashedPasswordInDBMatchesPassword = await user.checkPassword(password);
        ...
        it('should hash a new user\'s plain text password when saving the user to the db', async () => {
            ...
            const hashedPasswordMatchesPassword = await user.checkPassword(password);
